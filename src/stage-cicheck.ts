...
export function createCiCheckStageHandler(
  ...
  return {
    ...
    handler: async (ctx: StageContext): Promise<StageResult> => {
      ...
      while (true) {
        // Bail out promptly on Ctrl+C instead of finishing the poll
        // loop.  The thrown abort propagates out to the engine, which
        // unwinds through its normal cancelled path rather than
        // reporting a CI failure or timeout.
