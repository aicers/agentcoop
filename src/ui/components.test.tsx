...
describe("AgentPane", () => {
  ...
  test("active pane shows yellow dot indicator", () => {
    const emitter = new PipelineEventEmitter();
    const { lastFrame } = render(
      <Box>
        <AgentPane
    ...
    expect(frame).toContain("\u25CF");
    // Only the active pane should have the indicator.
    const dotCount = (frame.match(/\u25CF/g) ?? []).length;
    expect(dotCount).toBe(1);
  });

  test("title separator line renders between header and content", async () => {
    const emitter = new PipelineEventEmitter();
    const { lastFrame } = render(
      <AgentPane label="Agent A" agent="a" emitter={emitter} color="blue" />,
    );
    ...
    const frame = lastFrame() ?? "";
    // The horizontal rule character must appear as a separator.
    expect(frame).toContain("\u2500");
  });
});
